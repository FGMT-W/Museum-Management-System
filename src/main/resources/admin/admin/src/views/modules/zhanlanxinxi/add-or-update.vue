...
export default {
  ...
  methods: {
	...
	this.ruleForm.changguanjieshao = this.ruleForm.changguanjieshao.replace(reg,'../../../springboot5237s/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhanlanxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhanlanxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
