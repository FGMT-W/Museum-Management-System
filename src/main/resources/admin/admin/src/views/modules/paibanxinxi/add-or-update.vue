<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="工作编号" prop="gongzuobianhao">
          <el-select :disabled="ro.gongzuobianhao" @change="gongzuobianhaoChange" v-model="ruleForm.gongzuobianhao" placeholder="请选择工作编号">
            <el-option
                v-for="(item,index) in gongzuobianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.gongzuobianhao" label="工作编号" prop="gongzuobianhao">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.paibantupian" label="排班图片" prop="paibantupian">
          <file-upload
          tip="点击上传排班图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.paibantupian?ruleForm.paibantupian:''"
          @change="paibantupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.paibantupian" label="排班图片" prop="paibantupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.paibantupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='paibantupian'){
            this.ruleForm.paibantupian = obj[o];
	    this.ro.paibantupian = true;
            continue;
          }
          if(o=='paibanneirong'){
      ...
      this.$http({
		...
		if(this.$storage.get("role")!="管理员") {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/gongzuorenyuan/gongzuobianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.gongzuobianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    gongzuobianhaoChange () {
      this.$http({
        url: `follow/gongzuorenyuan/gongzuobianhao?columnValue=`+ this.ruleForm.gongzuobianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	if(this.ruleForm.paibantupian!=null) {
		this.ruleForm.paibantupian = this.ruleForm.paibantupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    paibantupianUploadChange(fileUrls) {
	this.ruleForm.paibantupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
